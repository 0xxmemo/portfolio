...
export function initSpaceCanvas(): void {
  ...
  function pickStarColor(): string {
    const r = Math.random();
    if (r < 0.6) return "rgba(200, 220, 255, ";
    if (r < 0.8) return "rgba(255, 240, 200, ";
    if (r < 0.9) return "rgba(0, 240, 255, ";
  ...
  const draw = () => {
    ...
    const warpSpeed = 12 * (1 - heroP * heroP) + 0.3;
    const streakLength = Math.max(0, (1 - heroP) * 8);
